...
| `utils.ts` | `formatFecha` (see date/time note below), `formatRangoFecha`, `formatProgramacion`, `describirProgramacion`, `calcularTextoRecurrente`, `formatDiasRecurrentes`, `calcularFinPorDefecto`, `esVisible`, `ordenarPorPosicion`, `inputValueAFechaHora`/`fechaAInputValue`/`horaAInputValue` |
| `useAvisos.ts` | Public realtime read (home carousel) — subscribes, filters by `esVisible`, sorts by `ordenarPorPosicion` |
| `useAvisosAdmin.ts` | Admin CRUD: form state, create/update/delete, inline confirm flow, fires `/api/avisos/notify` on create (never on update — editing an existing aviso does not re-notify); also `guardarOrden()`, the Firestore write side of manual reordering |

The drag-to-reorder gesture itself isn't avisos-specific code anymore — it's `useReordenarLista<T>()` in `src/hooks/useReordenarLista.ts` (see [Component conventions](#component-conventions) for why generic, reusable pieces like this get hoisted out of the feature folder). `ListaAvisosAdmin.tsx` is the only current caller, passing it `avisos` + `useAvisosAdmin.guardarOrden`.
...
| `ListaAvisosAdmin.tsx` | Admin history list orchestrator — calls `useReordenarLista` + `construirFilasArrastre` (both from `src/hooks/useReordenarLista.ts`) to turn `avisos` into rows, then maps each to `AvisoAdminCard.tsx` (or a thin "hueco" `<li>` drop-indicator bar) |
| `AvisoAdminCard.tsx` | One `<li>` in that list — `ui/DragHandle`, banner thumbnail if set, a left-edge color strip (`border-l-success` active / gray inactive, from `esVisible`) plus an "Inactivo" badge, `describirProgramacion` summary line, edit/delete buttons, inline "¿Eliminar?" confirm. Same "list items are their own component" shape as `peticiones/PeticionCard.tsx` — takes individual props, not the whole `admin` hook object |

Behavior:

- Reads/writes the Firestore `avisos` collection (last 50, ordered by `fechaCreacion` desc)
- **Ordering on the public carousel is fully manual** (`orden: number`, lower = first), set by dragging rows in "Avisos publicados" (`useReordenarLista` + `useAvisosAdmin.guardarOrden`, a `writeBatch` that only touches avisos whose index actually changed). `ordenarPorPosicion` (in `utils.ts`) sorts by `orden` ascending and is used by **both** `useAvisos` (public carousel) and `useAvisosAdmin` (admin list) — the admin list *is* the reorder UI, so what you drag there is exactly what shows on the homepage, in that order, `importante`/expiring/recurring status no longer affects sort order at all (it used to auto-pin `importante` avisos first and sort the rest by date — replaced per explicit user request for full manual control). A newly-published aviso gets `orden = min(existing) - 1` (goes to the very top of both lists); the admin can drag it anywhere after. Avisos from before this feature shipped have no `orden` until `scripts/migrar-orden-avisos.js` runs once (`node --env-file=.env.local scripts/migrar-orden-avisos.js` — assigns it by `fechaCreacion desc`, i.e. today's default admin-list order, so the migration doesn't visibly reshuffle anything); `ordenarPorPosicion` puts any doc still missing `orden` at the very end (by `fechaCreacion desc` among themselves) as a defensive fallback, but the migration script is the intended one-time fix, same pattern as `scripts/migrar-numeros.js` for `peticiones`
...
- **Banner (optional, URL only — no file upload, see [Firebase integration](#firebase-integration))**: when set, the carousel renders the image on top (rounded, `aspect-video`) with the "Importante" badge floating over it, and título/descripción/fecha in a solid panel **below** the image — not overlaid on top of it. (An earlier version overlaid white text on the image with a dark gradient scrim; replaced per explicit user request because arbitrary admin-uploaded images made the overlaid text look bad/illegible depending on the image's own colors and busyness. Putting the text on its own solid surface makes it legible regardless of what image is used.) If the URL fails to load, it falls back to the plain-text layout automatically (tracked per-aviso-id in local component state, not persisted)
- **Banner URL input has two modes** (`AvisoForm`, `OrigenBanner` in `types.ts`, `ORIGEN_BANNER_OPCIONES` in `constants.ts`): "URL pública" (a plain `ClearableTextInput`) or "Google Drive". Google Drive's own "compartir" link is an HTML page, not a direct image — it doesn't work as an `<img src>`. The "Google Drive" mode shows the fixed prefix `DRIVE_BANNER_BASE_URL` (`https://lh3.googleusercontent.com/d/`, in `constants.ts`) next to a `ClearableTextInput` where the admin pastes only the file ID; `construirBannerUrlDrive`/`extraerIdDriveDeBannerUrl` (`utils.ts`) build/parse the full `bannerUrl` from that ID. `bannerUrl` itself is still the only value persisted to Firestore — the two-mode UI is purely presentational and re-derives which mode to show from the current `bannerUrl` whenever `idEditando` changes (so editing an existing Drive-based aviso reopens in "Google Drive" mode with the ID pre-filled). The Drive file must be shared as "Cualquier persona con el enlace" for the direct link to load.
...
- **Drag-to-reorder** (`src/hooks/useReordenarLista.ts` — generic over `T extends { id: string }`, not avisos-specific, see [Component conventions](#component-conventions)) is hand-rolled with Pointer Events, not the native HTML5 Drag and Drop API and not a library — HTML5 DnD has no reliable touch support (a real requirement here, given this site's mobile-first admin UI), and Pointer Events cover mouse/touch/pen uniformly with `setPointerCapture` (so the drag keeps tracking even once the pointer leaves the row) plus `touch-none` on the handle (so touch-dragging doesn't also scroll the page — see `ui/DragHandle.tsx`). While dragging, only the dragged row moves (a `translateY` following the pointer); the rest of the list stays put and a thin `accent`-colored bar (`construirFilasArrastre`'s `"hueco"` row) shows where it would land. The actual reorder only happens on drop (`soltar()` in the hook), which splices the dragged item to its target index and hands the new array to the `onReordenar` callback (`ListaAvisosAdmin` wires this to `useAvisosAdmin.guardarOrden`). Every row reflow (including the drop) is animated with a manual FLIP (measure position before → let the DOM update → apply an inverse transform → transition it to zero) since a plain list reorder has nothing to interpolate on its own; the dragged row's last on-screen position (with its live drag transform still applied) is captured right before the drop so the FLIP animation continues from there instead of snapping back to its pre-drag spot first.
- **The left-edge color strip and its CSS pitfall**: `AvisoAdminCard`'s active/inactive indicator is `border-l-4` + `border-l-success`/`border-l-gray-*`. The card's other three sides must be colored with *directional* utilities too (`border-t-gray-200 border-r-gray-200 border-b-gray-200`, and the `dark:` equivalents) — never the bare `border-gray-200`/`dark:border-white/10` shorthand. That shorthand expands to the `border-color` CSS property, which sets `border-left-color` too; Tailwind emits all `dark:`-variant rules after the base rules in the stylesheet, so in dark mode a bare `dark:border-white/10` on the card silently overwrote the indicator's `border-l-*` color (same-specificity, later-in-source wins) — light mode had no competing `dark:` rule, which is why the bug only showed up in dark mode and was easy to miss. If a future indicator strip needs a differently-colored single side, keep every other side's color scoped to its own edge (`border-t-*`/`border-r-*`/`border-b-*`) rather than the bare/shorthand color utility.
...
```ts
{
  ...
  fechaCreacion: Timestamp,
  orden?: number,                  // manual drag position, lower = first; absent on pre-migration docs — see ordenarPorPosicion
...
- **Every `/api/*/notify` route sends `webpush: { headers: { Urgency: "high" }, fcmOptions: { link } }`.** Without an explicit high urgency, Android can defer delivery of a push until the device leaves Doze (battery-saving standby) — a real reported bug where notifications only ever seemed to "arrive" once the visitor opened the app themselves. Confirmed fixed with a real push received on a locked phone.
...
- **Notificaciones** — an **"Activar todo"** master `ui/Switch`, then one `ui/Switch` per topic (`SECCION_PETICIONES`/`SECCION_AVISOS`/`SECCION_CITAS` in `settings/constants.ts`, each backed by its own `useFcm(topic)` instance via `settings/useNotificationSettings.ts`) — all four rows live in the **same** `divide-y` list (not "Activar todo" in its own bordered box above a separate list), so the vertical gap is identical between every row; splitting them into two containers was a real reported bug (uneven spacing)
...
**"Activar todo"** (`alternarTodo` in `useNotificationSettings.ts`) subscribes/unsubscribes whichever of the three topics aren't already in the target state. It's guarded by a `useRef` flag (`enCursoRef`), checked and set synchronously before any `await` — a real reported bug: without that guard, tapping the switch again before the first pass finished let a second `alternarTodo()` call race the first through the same subscribe/unsubscribe loop, and the three topics ended up in an unpredictable combination. `cargandoTodo` (state, not just the ref) drives the master switch's `loading` prop and also force-disables the three individual switches while it runs, so nothing else can be toggled mid-operation either.
...
- `ui/Switch.tsx` is a reusable toggle-switch primitive (`role="switch"` button, not a styled checkbox) — use it for any future on/off setting instead of a bare `<input type="checkbox">`. Its optional `loading` prop shows a spinner (and implies disabled) — use it for any toggle whose underlying action is async and could otherwise leave the user unsure whether their tap registered.
...
- **Known limitation:** after the user updates in Play and taps "Abrir", Android can restore the old page (still holding the stale version), so the gate keeps showing. The page can't detect the new version until the app cold-starts, so once "Actualizar" is tapped the gate shows a "close the app completely and reopen it" hint (`fueALaTienda` in `useAppUpdate.ts`) instead of trying to auto-detect it.
...
- The modal card is `flex flex-col max-h-[85vh]` split into three regions — a `shrink-0` header (holds the close X), a `flex-1 min-h-0 overflow-y-auto` scrollable body (the `<ul>` of items), and a `shrink-0` footer ("Entendido") — **not** one unconstrained block. A real reported bug: once `NOVEDADES` grew past ~4 items it could exceed the viewport with no way to scroll, pushing the close button *and* "Entendido" off-screen with no way to dismiss the modal at all. Keep this three-region structure no matter how long the list gets.
...
- The tagline sits inside a small `max-w-md` block with a `text-xs font-bold uppercase tracking-widest text-accent` eyebrow label ("Nuestra misión") flanked by two fading accent lines above it — the same eyebrow-label convention used by `AvisosCarousel` ("Avisos") and `CitaBiblicaCard` ("Dios te habla hoy"). It used to be a single bare `text-2xl` sentence floating between the two card sections with no visual framing; replaced per explicit user feedback that it "felt unstructured." Follow this same eyebrow-plus-statement pattern for any future standalone one-line statement on the home page rather than dropping in bare text again.
...
- Each of those same three indexed routes sets its own `alternates: { canonical: "..." }` (the home page's lives on the root `layout.tsx` metadata, since `page.tsx` has no metadata export of its own to override it). **This does not auto-propagate** — Next only merges metadata fields a child route doesn't set from its parent, so if you give `/peticiones` or `/politica-privacidad` its own `metadata` export without a `canonical`, it would silently inherit `/` from the root layout instead of pointing at itself. Any newly-indexed page needs its own explicit `alternates.canonical`
- The home page (`src/app/page.tsx`) has a visually-hidden `<h1 className="sr-only">Centro Cristiano Jordán</h1>` at the top — added because the header only renders the church name as an image `alt` text (`logo-08-web.png`), so without it the page had no real `<h1>` at all (the first visible heading was the `<h2>Síguenos</h2>` card). Keep this `sr-only` h1 in place even if the visual design changes; don't remove it just because it isn't shown on screen
...
| `ClearableTextInput` | Text/URL/email/tel input with an inline "X" to clear it | Wraps `TextInput` + `CloseIcon`; the X only renders once `value` is non-empty and the field isn't `disabled`, calls the required `onClear` prop. Optional `clearAriaLabel` (default `"Borrar"`) for a more specific label, e.g. `"Limpiar búsqueda"`. This is the default for any single-line text-like field across the app now — título/nombre/referencia/correo/teléfono/búsqueda fields all use it (see `AvisoForm`, `CitaForm`, `NuevaPeticionForm`, `FiltrosPeticiones`, `AuthModal`'s email fields). Deliberately **not** used on `type="date"`/`"time"` inputs (the native picker already has its own clear affordance in most browsers, and stacking a custom one would overlap the native calendar icon) or on `PasswordInput` (its right-side slot is already the show/hide eye toggle) |
...
| `GripIcon` | Six-dot drag-handle SVG | Used by `DragHandle` |
| `DragHandle` | Drag handle button for any `useReordenarLista`-backed list | Wraps `GripIcon`; spread the `dragHandleProps(id)` a caller gets from `useReordenarLista` directly onto it as props. Used by `avisos/AvisoAdminCard.tsx` — see [Avisos](#avisos-announcements-feature) |
